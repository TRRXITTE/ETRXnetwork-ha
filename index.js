...
const traaittPlatformRPC = require('@trrxitte/traaittplatform-rpc').ETRXnetwork
...
const blockTargetTime = 644

const ETRXnetwork = function (opts) {
  opts = opts || {}
  if (!(this instanceof ETRXnetwork)) return new ETRXnetwork(opts)
  ...
  */
  ...
  // Begin ETRXnetwork options
  this.path = opts.path || path.resolve(__dirname, './ETRXnetwork' + ((os.platform() === 'win32') ? '.exe' : ''))
  this.dataDir = opts.dataDir || path.resolve(os.homedir(), './.traaittPlatform')
  this.logFile = opts.logFile || path.resolve(__dirname, './ETRXnetwork.log')
...
inherits(ETRXnetwork, EventEmitter)

ETRXnetwork.prototype.start = function () {
  ...
  this.emit('info', 'Attempting to start ETRXnetwork-ha...')
...
ETRXnetwork.prototype.stop = function () {
...
ETRXnetwork.prototype.write = function (data) {
  this._write(util.format('%s\r', data))
}

ETRXnetwork.prototype._checkChildStdio = function (data) {
...
ETRXnetwork.prototype._triggerDown = function () {
...
ETRXnetwork.prototype._triggerUp = function () {
...
ETRXnetwork.prototype._checkServices = function () {
  if (!this.synced) {
    this.synced = true
...
ETRXnetwork.prototype._checkRpc = function () {
  return Promise.all([
    this.api.info(),
    this.api.height()
  ])
...
ETRXnetwork.prototype._checkDaemon = function () {
...
ETRXnetwork.prototype._write = function (data) {
  this.child.write(data)
}

ETRXnetwork.prototype._buildargs = function () {
  var args = ''
...
ETRXnetwork.prototype._setupAPI = function () {
  this.api = new traaittPlatformRPC({
    host: this.rpcBindIp,
...
ETRXnetwork.prototype._setupWebSocket = function () {
...
ETRXnetwork.prototype._registerWebSocketClientEvents = function (socket) {
  var that = this
